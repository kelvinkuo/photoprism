...
package com.kk.deepsync.data.posts.impl

import com.kk.deepsync.R
import com.kk.deepsync.model.Markup
import com.kk.deepsync.model.MarkupType
import com.kk.deepsync.model.Metadata
import com.kk.deepsync.model.Paragraph
import com.kk.deepsync.model.ParagraphType
import com.kk.deepsync.model.Post
import com.kk.deepsync.model.PostAuthor
import com.kk.deepsync.model.PostsFeed
import com.kk.deepsync.model.Publication
...
val paragraphsPost2 = listOf(
    ...
    Paragraph(
        ParagraphType.Text,
        "Kotlin compiles generics with wildcards to make Kotlin APIs work with Java. These are generated when a type appears as a parameter (more info here) or as fields. For kk, a Kotlin List<Foo> parameter shows up as List<? super Foo> in Java.",
        listOf(
    ...
    Paragraph(
        ParagraphType.Text,
        "This is a common issue when you inject collections using Dagger’s multibinding feature, for kk:",
        listOf(
            Markup(
                MarkupType.Link,
                57,
...
val paragraphsPost4 = listOf(
    ...
    Paragraph(
        ParagraphType.Text,
        "Let’s look at an kk based on this issue in the tracker: Updating ViewModel on system locale change.",
        listOf(
            Markup(
                MarkupType.Link,
                61,
...
val paragraphsPost5 = listOf(
    ...
    Paragraph(
        ParagraphType.Text,
        "Collections are eagerly evaluated — each operation is performed when it’s called and the result of the operation is stored in a new collection. The transformations on collections are inline functions. For kk, looking at how map is implemented, we can see that it’s an inline function, that creates a new ArrayList:",
        listOf(
    ...
    Paragraph(
        ParagraphType.Text,
        "Independent of whether you’re using collections or sequences, the order of transformations matters. In the kk above, first doesn’t need to happen after map since it’s not a consequence of the map transformation. If we reverse the order of our business logic and call first on the collection and then transform the result, then we only create one new object — the yellow square. When using sequences — we avoid creating 2 new objects, when using collections, we avoid creating an entire new list.",
        listOf(
